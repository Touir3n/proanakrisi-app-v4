...
function getTimeRange(startId, endId, fallbackStart = "doc_start") {
    let s = document.getElementById(startId) ? document.getElementById(startId).value.trim() : "";
    if (!s) s = document.getElementById(fallbackStart) ? document.getElementById(fallbackStart).value.trim() : "";
    let e = document.getElementById(endId) ? document.getElementById(endId).value.trim() : "";
    
    if (!e && s && s.match(/^\d{2}:\d{2}$/)) {
        ...
        e = d.getHours().toString().padStart(2, '0') + ":" + d.getMinutes().toString().padStart(2, '0');
    }
...
function getD() {
    let v = function(id) {
        if (id.endsWith("_end")) {
            ...
            if (val) return val;
            let startId = id.replace("_end", "_start");
            if (startId === "doc_end") startId = "doc_start"; // fallback
            let sEl = document.getElementById(startId);
            let startVal = sEl ? sEl.value.trim() : "";
            if (!startVal && id === "doc_end") startVal = document.getElementById("doc_start") ? document.getElementById("doc_start").value.trim() : "";
            if (!startVal && startId !== "doc_start") startVal = document.getElementById("doc_start") ? document.getElementById("doc_start").value.trim() : ""; // fallback to doc_start
            
            if (startVal && startVal.match(/^\d{2}:\d{2}$/)) {
                let durationStr = document.getElementById("doc_duration") ? document.getElementById("doc_duration").value.trim() : "15";
                let duration = parseInt(durationStr) || 15;
    ...
    return {
        ...
        a_ereuna: g === 'M' ? "Ο καθ' ου η έρευνα και κατάσχεση" : "Η καθ' ης η έρευνα και κατάσχεση",
        a_paradous: g === 'M' ? "Ο ΠΑΡΑΔΟΥΣ" : "Η ΠΑΡΑΔΟΥΣΑ",
        a_gnostopoihsh: g === 'M' ? "Ο κατά του οποίου η γνωστοποίηση" : "Η κατά της οποίας η γνωστοποίηση",
        a_lavon_upoptos: g === 'M' ? "Ο λαβών γνώση Ύποπτος" : "Η λαβούσα γνώση Ύποπτη",
...
function exportToWord() {
    ...
    let relDirect = rel;
    if (rel === "τον κατηγορούμενο δε γνωρίζει") relDirect = "Όχι, δεν τον γνωρίζω.";
    else if (rel === "τον κατηγορούμενο γνωρίζει (απλά)") relDirect = "Ναι, τον γνωρίζω απλά.";
    else if (rel === "τον κατηγορούμενο γνωρίζει ως γείτονα") relDirect = "Ναι, είναι γείτονάς μου.";
    else if (rel === "τον κατηγορούμενο γνωρίζει φιλικά") relDirect = "Ναι, είναι φίλος / γνωστός μου.";
    else if (rel === "τυγχάνει συγγενής του κατηγορουμένου") relDirect = "Ναι, είναι συγγενής μου.";

    let body = `<p style="${pStyle}">Στην ${d.city}, σήμερα την ${d.dateStr} και ώρα ${d.v("doc_start")} ενώπιον εµού του ${d.anakr} του ${d.dept}, παρισταμένου και του ${d.banakr} της ίδιας υπηρεσίας, που προσλήφθηκε ως Β' Ανακριτικός Υπάλληλος, εμφανίσθηκε ${d.a_o} κατωτέρω μάρτυρας, ${d.a_os}, αφού ερωτήθηκε για την ταυτότητά ${d.a_tou} κλπ., απάντησε ότι ονομάζεται ${d.prof}.</p>
    ...
    <p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> Γνωρίζεις και συγγενεύεις με τον κατηγορούμενο, ή τον αδικηθέντα;</p>
    <p style="${pStyle}"><b>ΑΠΟΚΡΙΣΗ:</b> ${relDirect}</p>
...
function exportWeighing() {
    ...
    let signTitle = (d.v('gender') === 'M' ? 'Ο ΛΑΒΩΝ ΓΝΩΣΗ' : 'Η ΛΑΒΟΥΣΑ ΓΝΩΣΗ');
    ...
    <p style="${pStyleLast}">Για πίστωση συντάχθηκε η παρούσα έκθεση, η οποία αφού αναγνώσθηκε και βεβαιώθηκε, υπογράφεται ως ακολούθως:</p>
    ${sigBlock(signTitle, "Ο Β’ ανακριτικός υπάλληλος", "Ο ανακριτικός υπάλληλος")}`;
...
function resetExportCounter() {
    ...
    if (typeof showToast === 'function') {
        showToast("Η αρίθμηση των εγγράφων μηδενίστηκε (ξανά από το 1).");
    } else {
        alert("Η αρίθμηση των εγγράφων μηδενίστηκε (ξανά από το 1).");
    }
}

function getInterpreterProfileText(caseType = 'nom') {
    let dataStr = localStorage.getItem('profile_Διερμηνέας');
    if (!dataStr) return null;
    let data = JSON.parse(dataStr);
    
    let v = key => data[key] ? data[key].trim() : '';
    let g = data.gender || 'M';
    
    // Quick helper for declension (simplified or calling ui.js declineGreek if available)
    // Actually, export.js runs in same context as ui.js, so we can use declineGreek!
    ...
    let isFor = data.is_foreigner;
    let natStr = (isFor && v('nationality')) ? ', υπήκοος ' + v('nationality') : '';
...
function exportTestimonyInterpreter() {
    if (!validateRequiredFields(['surname', 'name', 'doc_testimony_simple', 'interp_lang'])) return;
    let interpProfAcc = getInterpreterProfileText('acc');
    let interpProfNom = getInterpreterProfileText('nom');
    if (!interpProfNom) {
        alert("Δεν βρέθηκε αποθηκευμένο προφίλ Διερμηνέα! Παρακαλώ φορτώστε τα στοιχεία του διερμηνέα και πατήστε 'Αποθήκευση: Ως Διερμηνέας' πρώτα.");
        return;
    }
    
    let d = getD(); let rel = d.v("doc_relation_simple"); let copy = d.v("doc_copy_simple"); let parav = d.v("doc_paravolo_simple");
    let header = `<p style="text-align: center; font-weight: bold; text-decoration: underline; font-family: 'Times New Roman'; font-size: 14pt; margin-bottom: 6pt;">ΕΚΘΕΣΗ ΕΝΟΡΚΗΣ ΕΞΕΤΑΣΗΣ ΜΑΡΤΥΡΑ ΜΕ ΔΙΕΡΜΗΝΕΑ ( Κ.Π.Δ. )</p>`;
    
    let lang = d.v("interp_lang") || ".........";
    let interpG = JSON.parse(localStorage.getItem('profile_Διερμηνέας') || '{}').gender || 'M';
    let interpTon = interpG === 'M' ? 'τον' : 'την';

    let body = `<p style="${pStyle}">Στην ${d.city} σήμερα την ${d.dateStr} και ώρα ${d.v("doc_start")} ενώπιον εμού του ${d.anakr} υπηρετούντος στο ${d.deptFull}, παρισταμένου και του ${d.banakr} της ιδίας Υπηρεσίας, προσληφθέντος ως Β΄ Ανακριτικός Υπάλληλος, κλήθηκε και εμφανίσθηκε ενώπιόν μας, ${d.a_o} ${d.a_kato} μάρτυρας, για ${d.a_ton} οποί${d.a_o} διαπιστώθηκε ότι αγνοεί την Ελληνική γλώσσα και ότι γνωρίζει την ${lang} (γλώσσα), προσλάβαμε ως διερμηνέα της Ελληνικής στην ${lang} και αντίστροφα, ${interpTon} ${interpProfAcc}, ${interpTon} οποί${interpG==='M'?'o':'a'} αφού ορκίσαμε κατά το άρθρο 236 Κ.Π.Δ. ότι θα μεταφράσει με ακρίβεια και πιστότητα, όλα όσα θα ειπωθούν κατά τη συζήτηση, προβήκαμε στην εξέταση ${d.a_tou} μάρτυρα ως έπεται :</p>
    <p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> Πως ονομάζεται κ.λ.π. (πλήρη στοιχεία ταυτότητας);</p>
    <p style="${pStyle}"><b>ΑΠΟΚΡΙΣΗ:</b> ${d.prof}. Ενημερώθηκε ότι μπορεί να λαμβάνει εγκαίρως γνώση των εγγράφων της δίκης, τα οποία του επιδίδονται και με ηλεκτρονικά μέσα σύμφωνα με τις παρ. 1 και 4 του άρθρου 155 Κ.Π.Δ.</p>
    
    <p style="${pStyle}">Έπειτα ${d.a_o} μάρτυρας, αφού έδωσε τον ακόλουθο όρκο: «Δηλώνω, επικαλούμενος την τιμή και τη συνείδησή μου, ότι θα πω όλη την αλήθεια και μόνο την αλήθεια, χωρίς να προσθέσω ούτε να αποκρύψω τίποτε», σύμφωνα με τα άρθρα 219 και 220 Κ.Π.Δ. στη συνέχεια εξετάζεται, ως ακολούθως :</p>
    
    <p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> Γνωρίζεις και συγγενεύεις με τον κατηγορούμενο, ή τον αδικηθέντα;</p>
    <p style="${pStyle}"><b>ΑΠΟΚΡΙΣΗ:</b> ${rel}</p>
    ...
    body += `<p style="${pStyle}">Κάτι άλλο δεν έχω να προσθέσω και υπογράφω.</p>
    <p style="${pStyle}">Η παρούσα έκθεση άρχισε να συντάσσεται την ${d.v("doc_start")} ώρα και περατώθηκε την ${d.v("doc_end")} ώρα.</p>
    <p style="${pStyleLast}">Για πιστοποίηση συντάχθηκε η παρούσα έκθεση και υπογράφεται ως ακολούθως, αφού πρώτα διερμηνεύθηκε στον μάρτυρα, από τον διερμηνέα το περιεχόμενό της και βεβαιώθηκε απ’ αυτόν.</p>
    
    <table width="100%" style="font-family: 'Times New Roman'; font-size: 12pt; text-align: center; margin-top: 20pt; border-collapse: collapse;">
        <tr>
            <td width="25%">Ο/Η Εξετασθείς</td>
            <td width="25%">Ο/Η Διερμηνέας</td>
            <td width="25%">Ο Β' Ανακρ. Υπάλλ.</td>
            <td width="25%">Ο Ανακρ. Υπάλληλος</td>
        </tr>
    </table>`;
    
    makeDoc("Ένορκη με Διερμηνέα", header, body, `ΕΚΘΕΣΗ_ΕΝΟΡΚΗ_ΜΑΡΤΥΡΑ_ΔΙΕΡΜΗΝΕΑΣ_${d.v("surname")}.doc`);
}

function exportApologiaInterpreter() {
    if (!validateRequiredFields(['surname', 'name', 'apologia_charge_short', 'apologia_plea', 'interp_lang'])) return;
    let interpProfAcc = getInterpreterProfileText('acc');
    let interpProfNom = getInterpreterProfileText('nom');
    if (!interpProfNom) {
        alert("Δεν βρέθηκε αποθηκευμένο προφίλ Διερμηνέα! Παρακαλώ φορτώστε τα στοιχεία του διερμηνέα και πατήστε 'Αποθήκευση: Ως Διερμηνέας' πρώτα.");
    ...
    let d = getD();
    let fullCharge = d.v("apologia_charge_short"); if(d.v("apologia_charge_details")) fullCharge += " - " + d.v("apologia_charge_details");
    
    let header = `<p style="text-align: center; font-weight: bold; text-decoration: underline; font-family: 'Times New Roman'; font-size: 14pt; margin-bottom: 6pt;">ΕΚΘΕΣΗ ΕΞΕΤΑΣΗΣ ΚΑΤΗΓΟΡΟΥΜΕΝΟΥ ΜΕ ΔΙΕΡΜΗΝΕΑ (Κ.Π.Δ.)</p>`;
    
    let lang = d.v("interp_lang") || ".........";
    let interpG = JSON.parse(localStorage.getItem('profile_Διερμηνέας') || '{}').gender || 'M';
    let interpTon = interpG === 'M' ? 'τον' : 'την';

    let body = `<p style="${pStyle}">Στην ${d.city} σήμερα την ${d.dateStr} και ώρα ${d.v("apo_start")} ενώπιον εμού του ${d.anakr} υπηρετούντος στο ${d.deptFull}, παρισταμένου και του ${d.banakr} της ιδίας Υπηρεσίας, κάτοικοι ${d.city} επειδή διαπιστώσαμε ότι ${d.a_o} ${d.a_kato} αγνοεί την ελληνική γλώσσα και ότι ομιλεί την ${lang} προσλάβαμε ως διερμηνέα ${interpTon} ${interpProfAcc}, ${interpTon} οποί${interpG==='M'?'o':'a'} αφού ορκίσαμε κατά το άρθρο 236 του Κώδικα Ποινικής Δικονομίας, ότι θα διερμηνεύσει ακριβώς και πιστά όλα όσα λεχθούν κατά τη συζήτηση, προβήκαμε μ’ αυτήν στην εξέταση του κατηγορουμένου ως ακολούθως: <b>ΕΡΩΤΗΣΗ:</b> Πώς ονομάζεται κ.λ.π. <b>ΑΠΟΚΡΙΣΗ:</b> ${d.prof}. Ενταύθα γνωρίσαμε στον εξεταζόμενο ότι κατηγορείται για ${fullCharge}.</p>
    <p style="${pStyle}">Στη συνέχεια και αφού ανακοινώσαμε σε ${d.a_auton} το περιεχόμενο των εγγράφων της ανάκρισης, εξηγήσαμε σ' ${d.a_auton}, κατ’ εφαρμογή των <b>άρθρων 95 & 273</b> του Κ.Π.Δ., με σαφήνεια όλα τα δικαιώματά ${d.a_tou}, που προβλέπονται από τις παραγράφους <b>1 & 2 του άρθρου 105</b> του Κ.Π.Δ. και ειδικότερα ότι ο κατηγορούμενος:</p>
    <p style="${pStyle}">Κατά το άρθρο 89 του Κ.Π.Δ., δεν μπορεί να αντιπροσωπεύεται ή να συμπαρίσταται στην ποινική διαδικασία με περισσότερους από δύο συνηγόρους στην προδικασία και τρεις στο ακροατήριο. Ο διορισμός συνηγόρου του γίνεται: α) με προφορική του δήλωση που καταχωρίζεται κατά την απολογία του ή β) με έγγραφη δήλωση του ιδίου (κατά τις διατυπώσεις του άρθρου 42 παρ. 2 εδ. β’ και γ του Κ.Π.Δ.) στην οποία η γνησιότητα της υπογραφής του πρέπει να βεβαιώνεται από οποιαδήποτε δημόσια, δημοτική ή κοινοτική αρχή ή δικηγόρο και η οποία προσαρτάται στην απολογία του κατηγορουμένου ή γ) με την έκθεση ή τη δήλωση άσκησης ενδίκου μέσου. Ο διορισμός παρέχει στον συνήγορο την εξουσία να εκπροσωπεί τον εντολέα του σε όλες τις διαδικαστικές πράξεις που αφορούν τη συγκεκριμένη ποινική υπόθεση, εκτός αν η πληρεξουσιότητα παρέχεται για ορισμένες μόνο από τις πράξεις αυτές. Η γενική πληρεξουσιότητα περιλαμβάνει την άσκηση ενδίκων μέσων, εφόσον αυτό μνημονεύεται ρητά.</p>
    <p style="${pStyle}">Κατά το άρθρο 91 του Κ.Π.Δ., έχει δικαίωμα δωρεάν νομικής βοήθειας το οποίο περιλαμβάνει παροχή νομικών συμβουλών και νομική αρωγή και εκπροσώπηση του ενώπιον του δικαστηρίου, σύμφωνα με όσα ορίζονται στον Ν. 3226/2004 (Νομική βοήθεια σε πολίτες χαμηλού εισοδήματος)σύμφωνα με τον οποίο δικαιούχοι νομικής βοήθειας είναι οι χαμηλού εισοδήματος πολίτες κράτους - μέλους της Ευρωπαϊκής Ένωσης, καθώς επίσης και οι χαμηλού εισοδήματος πολίτες τρίτου κράτους και ανιθαγενείς, εφόσον έχουν, νομίμως, κατοικία ή συνήθη διαμονή στην Ευρωπαϊκή Ένωση. Κατά τον ανωτέρω νόμο, πολίτες χαμηλού εισοδήματος, δικαιούχοι νομικής βοήθειας, είναι εκείνοι των οποίων το ετήσιο οικογενειακό εισόδημα δεν υπερβαίνει τα δύο τρίτα των κατώτατων ετήσιων ατομικών αποδοχών που προβλέπει η Εθνική Γενική Συλλογική Σύμβαση Εργασίας. Σε περίπτωση ενδοοικογενειακής διαφοράς ή διένεξης, δεν λαμβάνεται υπόψη το εισόδημα εκείνου με τον οποίο υπάρχει η διαφορά ή διένεξη. Επίσης, σύμφωνα με τον ανωτέρω νόμο, η νομική βοήθεια παρέχεται ύστερα από αίτηση του δικαιούχου, η οποία θα αναφέρει συνοπτικά το αντικείμενο της δίκης ή της πράξης και τα στοιχεία που βεβαιώνουν τη συνδρομή των προϋποθέσεων για την παροχή της βοήθειας. Στην αίτηση επισυνάπτονται τα αναγκαία δικαιολογητικά αποδεικτικά της οικονομικής καταστάσεως (ιδίως αντίγραφο φορολογικής δήλωσης ή βεβαίωσης του εφόρου ότι δεν υποχρεούται σε υποβολή δήλωσης, αντίγραφο δήλωσης περιουσιακής καταστάσεως, εκκαθαριστικού σημειώματος, Α.Φ.Μ., βεβαιώσεις υπηρεσιών κοινωνικής πρόνοιας, ένορκες βεβαιώσεις) και αποδεικτικά της κατοικίας ή διαμονής, εάν πρόκειται για πολίτη τρίτου κράτους. Η νομική βοήθεια μπορεί να ανακληθεί ή να περιοριστεί με απόφαση του αρμοδίου, αυτεπαγγέλτως, εφόσον αποδεικνύεται ότι οι προϋποθέσεις της παροχής είτε δεν υπήρχαν είτε εξέλιπαν είτε μεταβλήθηκαν ουσιωδώς ενώ μαζί με την ανάκληση μπορεί να επιβληθεί στον αιτούντα που πέτυχε την παροχή της βοήθειας με αναληθή αίτηση ή στοιχεία χρηματική ποινή από δεκαπέντε (15) έως εκατόν πενήντα (150) ευρώ. Η χρηματική ποινή δεν θίγει την υποχρέωση καταβολής των ποσών από τα οποία απαλλάχθηκε.</p>
    <p style="${pStyle}">Κατά το άρθρο 96§1 του Κ.Π.Δ., στον κατηγορούμενο παρέχεται αμέσως έγγραφο στο οποίο καταγράφονται τα δικαιώματα του, όπως: α) το δικαίωμα παράστασης με συνήγορο, β) το δικαίωμα και τις προϋποθέσεις παροχής δωρεάν νομικών συμβουλών, γ) το δικαίωμα ενημέρωσης σχετικά με την κατηγορία, δ) το δικαίωμα διερμηνείας και μετάφρασης, ε) το δικαίωμα σιωπής και μη αυτοενοχοποίησης, στ) το δικαίωμα πρόσβασης στο υλικό της δικογραφίας, ζ) το δικαίωμα ενημέρωσης των προξενικών αρχών και ενός επιπλέον προσώπου της επιλογής του, η) το δικαίωμα σε επείγουσα ιατρική περίθαλψη, θ) τον ανώτατο αριθμό ωρών ή ημερών κατά τις οποίες ο κατηγορούμενος δύναται να στερηθεί της ελευθερίας του προτού προσαχθεί ενώπιον δικαστικής αρχής και ι) πληροφορίες σχετικά με τις δυνατότητες προσβολής του νόμιμου χαρακτήρα της σύλληψης ή της κράτησης.</p>
    <p style="${pStyle}">Κατά το άρθρο 97§1 του Κ.Π.Δ., έχει δικαίωμα να ζητήσει να ενημερωθεί, χωρίς αδικαιολόγητη καθυστέρηση, για τη στέρηση της ελευθερίας του ένα τουλάχιστον πρόσωπο της επιλογής του. Αν ο κατηγορούμενος είναι ανήλικος ενημερώνεται ο ασκών τη γονική μέριμνα, εκτός αν αυτό αντιβαίνει στο συμφέρον του ανηλίκου, οπότε ενημερώνεται κάποιο άλλο ενδεδειγμένο ενήλικο πρόσωπο ή η αρμόδια για την προστασία των ανηλίκων αρχή ενώ σύμφωνα με την παρ. 2 του άρθρου 97 του Κ.Π.Δ. σε εξαιρετικές περιπτώσεις και για να αποτραπεί άμεσος κίνδυνος είτε για τη ζωή, την ελευθερία ή τη σωματική ακεραιότητα κάποιου προσώπου είτε για την αποδεικτική διερεύνηση του εγκλήματος, οι αρμόδιες αρχές μπορούν να μην επιτρέπουν προσωρινά την ενημέρωση τρίτου προσώπου για τη στέρηση της ελευθερίας του κατηγορουμένου. Στην περίπτωση αυτή εξετάζεται αν ένα άλλο τρίτο πρόσωπο, που έχει υποδειχθεί από τον κατηγορούμενο, μπορεί να ενημερωθεί σχετικά. Στην περίπτωση που ο κατηγορούμενος είναι ανήλικος, ενημερώνεται η αρμόδια για την προστασία των ανηλίκων αρχή.</p>
    <p style="${pStyle}">Κατά το άρθρο 98§1 του Κ.Π.Δ., ο κατηγορούμενος που στερείται την ελευθερία του έχει δικαίωμα επικοινωνίας, χωρίς αδικαιολόγητη καθυστέρηση, με ένα τουλάχιστον τρίτο πρόσωπο που έχει υποδείξει ο ίδιος. Σε εξαιρετικές περιπτώσεις και για να αποτραπεί ο άμεσος κίνδυνος της παρ. 2 του προηγούμενου άρθρου, οι αρμόδιες αρχές μπορούν να περιορίζουν ή να αναβάλλουν την άσκηση του ανωτέρω δικαιώματος. Στην περίπτωση αυτή εξετάζεται πρώτα αν ο κατηγορούμενος μπορεί να επικοινωνήσει με ένα άλλο πρόσωπο που αυτός υποδεικνύει.</p>
    <p style="${pStyle}">Κατά το άρθρο 100 του Κ.Π.Δ., ο Ανακριτικός Υπάλληλος, μόλις οδηγηθεί σε αυτόν ο κατηγορούμενος για να απολογηθεί, του ανακοινώνει το περιεχόμενο του κατηγορητηρίου και των εγγράφων της προανάκρισης. Επιτρέπεται επίσης στον κατηγορούμενο να μελετήσει ο ίδιος ή ο συνήγορος του το κατηγορητήριο και τα έγγραφα της προανάκρισης. Με γραπτή αίτηση του κατηγορουμένου και με δαπάνη του, χορηγούνται σε αυτόν αντίγραφα του κατηγορητηρίου και των εγγράφων της προανάκρισης. Την ίδια υποχρέωση έχει ο Ανακριτικός Υπάλληλος και τα ίδια δικαιώματα ο κατηγορούμενος, όταν κληθεί ξανά σε συμπληρωματική απολογία. Πάντως μετά το τέλος της προανάκρισης και προτού διαβιβαστεί η δικογραφία στον Εισαγγελέα, καλείται πάντοτε ο κατηγορούμενος να μελετήσει όλη τη δικογραφία. Αν όμως η προανάκριση εξακολούθησε περισσότερο από μήνα μετά την πρώτη ή κάθε μεταγενέστερη απολογία, δικαιούται ο κατηγορούμενος να ασκεί τα δικαιώματά του μία φορά το μήνα, και κάθε φορά ο Ανακριτικός Υπάλληλος συντάσσει σχετική έκθεση κάτω από την απολογία του κατηγορουμένου.</p>
    <p style="${pStyle}">Κατά το άρθρο 101 του Κ.Π.Δ., ο κατηγορούμενος, που δεν ομιλεί ή δεν κατανοεί επαρκώς την ελληνική γλώσσα, έχει το δικαίωμα σε διερμηνεία καθώς και σε γραπτή μετάφραση όλων των ουσιωδών εγγράφων της διαδικασίας σύμφωνα με όσα προβλέπονται στα άρθρα 233 και 237 του Κ.Π.Δ..</p>
    <p style="${pStyle}">Κατά το άρθρο 103§1 του Κ.Π.Δ., ο κατηγορούμενος έχει το δικαίωμα να ζητήσει προθεσμία τουλάχιστον σαράντα οκτώ ωρών και δεν έχει υποχρέωση να απολογηθεί πριν περάσει η προθεσμία. Ο Ανακριτικός Υπάλληλος μπορεί να παρατείνει την ανωτέρω προθεσμία ύστερα από αίτηση του κατηγορουμένου.</p>
    <p style="${pStyle}">Κατά το άρθρο 104 του Κ.Π.Δ., ο κατηγορούμενος έχει το δικαίωμα της σιωπής και μη αυτοενοχοποίησης. Η άσκηση του δικαιώματος της μη αυτοενοχοποίησης δεν εμποδίζει τη νόμιμη συγκέντρωση αποδεικτικών στοιχείων, που υπάρχουν, ανεξάρτητα από τη βούληση των υπόπτων και των κατηγορουμένων, καθώς επίσης, η άσκηση του ανωτέρω δικαιώματος δεν μπορεί να αξιοποιηθεί σε βάρος του κατηγορουμένου.</p>
    <p style="${pStyle}">Κατά το άρθρο 105§2 του Κ.Π.Δ., ο κατηγορούμενος έχει το δικαίωμα πρόσβασης σε δικηγόρο χωρίς αδικαιολόγητη καθυστέρηση και σε κάθε περίπτωση προτού εξεταστεί από την αστυνομία ή άλλη αρχή επιβολής του νόμου ή δικαστική αρχή. Σε καμία περίπτωση δεν μπορεί να απαγορευθεί η επικοινωνία του κατηγορουμένου με τον συνήγορό του. Η επικοινωνία αυτή είναι απολύτως απόρρητη.</p>
    <p style="${pStyle}">Κατά το άρθρο 273§2 του Κ.Π.Δ., έχει δικαίωμα κατά την εξέτασή του να αρνηθεί να απαντήσει και να παραδώσει την απολογία του γραπτή.</p>
    <p style="${pStyle}">Κατά το άρθρο 279 του Κ.Π.Δ. ο συλλαμβανόμενος επ' αυτοφώρω ή με ένταλμα οδηγείται χωρίς αναβολή στον αρμόδιο εισαγγελέα, το αργότερο μέσα σε είκοσι τέσσερις (24) ώρες από τη σύλληψή του και, αν η σύλληψη έγινε έξω από την έδρα του, στον απολύτως αναγκαίο χρόνο για την μεταφορά του.</p>
    <p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> Επιθυμείτε να κάνετε χρήση των δικαιωμάτων που σας γνωστοποιήθηκαν;</p>
    <p style="${pStyle}"><b>ΑΠΟΚΡΙΣΗ:</b> ${d.v("apologia_rights").replace(/[\n\r]+/g, " ")}</p>
    <p style="${pStyle}">Ύστερα από τα ανωτέρω προβήκαμε στη λήψη της απολογίας ${d.a_tou}. Για πιστοποίηση συντάχθηκε η παρούσα έκθεση, η οποία αφού αναγνώστηκε και βεβαιώθηκε υπογράφεται ως ακολούθως :</p>
    
    <table width="100%" style="font-family: 'Times New Roman'; font-size: 12pt; text-align: center; margin-top: 20pt; border-collapse: collapse; margin-bottom: 20pt;">
        <tr>
            <td width="25%">Ο Εξετασθείς</td>
            <td width="25%">Ο Διερμηνέας</td>
            <td width="25%">Ο Β' Ανακρ. Υπάλληλος</td>
            <td width="25%">Ο Ανακρ. Υπάλληλος</td>
        </tr>
    </table>
    
    <p style="${pStyle}">Στη συνέχεια, προβήκαμε στην εξέταση του κατηγορουμένου ως ακολούθως:</p>
    <p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> Κατηγορηθήκατε άλλη φορά και για ποια αιτία;</p>
    <p style="${pStyle}"><b>ΑΠΟΚΡΙΣΗ:</b> Όχι ποτέ</p>
    <p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> Κατηγορείστε ήδη για τις πράξεις που σας γνωστοποιήθηκαν ανωτέρω. Τι απολογείστε;</p>
    ${formatTextToParagraphs("ΑΠΟΚΡΙΣΗ: " + d.v("apologia_plea"))}
    <p style="${pStyle}">Στον κατηγορούμενο γνωστοποιήσαμε ότι, σύμφωνα με το άρθρο 273 § 1 του Κ.Π.Δ., υποχρεούται να δηλώσει κάθε μεταβολή της κατοικίας ή της διαμονής του ή της διεύθυνσης ηλεκτρονικού ταχυδρομείου ή αριθμού σύνδεσης κινητής τηλεφωνίας, εγγράφως στον Εισαγγελέα που άσκησε την ποινική δίωξη. Για πίστωση συντάχθηκε η παρούσα έκθεση, η οποία άρχισε την ${d.v("apo_start")} ώρα της ${d.dateStr} περατώθηκε την ${d.v("apo_end")} ώρα της ιδίας και αφού αναγνώσθηκε και βεβαιώθηκε υπογράφεται ως ακολούθως</p>
    ...
    makeDoc("Απολογία με Διερμηνέα", header, body, `ΕΚΘΕΣΗ_ΑΠΟΛΟΓΙΑΣ_ΔΙΕΡΜΗΝΕΑΣ_${d.v("surname")}.doc`);
}
