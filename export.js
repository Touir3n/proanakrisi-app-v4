...
// Υπογραφές (0 κενά πάνω, 3 κενά κάτω)
...
// Ο Γραμματικός Κινητήρας (Grammar Engine) - Εξαλείφει τα διπλά γράμματα!
function getD() {
...
function exportToWord() {
    ...
    body += `<p style="${pStyle}">Κάτι άλλο δεν έχω να προσθέσω και υπογράφω.</p><p style="${pStyle}">Η παρούσα έκθεση άρχισε να συντάσσεται την ${d.v("doc_start")} ώρα και περατώθηκε την ${d.v("doc_end")} ώρα.</p><p style="${pStyle}">Ενημερώθηκε ότι μπορεί να λαμβάνει εγκαίρως γνώση των εγγράφων της δίκης, τα οποία επιδίδονται και με ηλεκτρονικά μέσα σύμφωνα με τις παρ. 1 και 4 του άρθρου 155 Κ.Π.Δ.</p><p style="${pStyle}">Για πιστοποίηση συντάχθηκε η παρούσα έκθεση, η οποία αφού πρώτα αναγνώστηκε και βεβαιώθηκε υπογράφεται ως ακολούθως:</p>${sigBlock(d.a_sign, "Ο Β’ Ανακριτικός Υπάλληλος", "Ο Ανακριτικός Υπάλληλος")}`;
...
function exportToWordNoOath() {
    ...
    let body = `<p style="${pStyle}">Στην ${d.city}, σήμερα την ${d.dateStr} και ώρα ${d.v("doc_start")} ενώπιον εμού του ${d.anakr} του ${d.dept}, παρισταμένου και του ${d.banakr} της ίδιας υπηρεσίας, που προσλήφθηκε ως Β' Ανακριτικός Υπάλληλος, εμφανίσθηκε ${d.a_o} κατωτέρω μάρτυρας, ${d.a_os}, αφού ερωτήθηκε για την ταυτότητά ${d.a_tou} κλπ., απάντησε ότι ονομάζεται ${d.prof} και ότι ${rel}.</p><p style="${pStyle}">Έπειτα ${d.a_o} ${d.a_exet}, ως μέλος οικογένειας σε υπόθεση ενδοοικογενειακής βίας, εξετάζεται χωρίς όρκο δυνάμει του άρθρου 19 παρ. 1 Ν.3500/2006 ως ακολούθως:</p><p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> Τι προσήλθατε να καταθέσετε στην Υπηρεσία μας;</p>${formatTextToParagraphs("<b>ΑΠΟΚΡΙΣΗ:</b> " + d.v("dv_q_main"))}`;
    dvQuestions.forEach((q, i) => { let ans = d.v("dv_q_" + i); if (ans) body += `<p style="${pStyle}"><b>ΕΡΩΤΗΣΗ:</b> ${q}</p>${formatTextToParagraphs("<b>ΑΠΟΚΡΙΣΗ:</b> " + ans)}`; });
    ...
    body += `<p style="${pStyle}">Η παρούσα έκθεση άρχισε να συντάσσεται την ${d.v("doc_start")} ώρα και περατώθηκε την ${d.v("doc_end")} ώρα.</p><p style="${pStyle}">Ενημερώθηκε ότι μπορεί να λαμβάνει εγκαίρως γνώση των εγγράφων της δίκης, τα οποία επιδίδονται και με ηλεκτρονικά μέσα σύμφωνα με τις παρ. 1 και 4 του άρθρου 155 Κ.Π.Δ.</p><p style="${pStyle}">Για πιστοποίηση συντάχθηκε η παρούσα έκθεση, η οποία αφού πρώτα αναγνώστηκε και βεβαιώθηκε από όλους του παριστάμενους, υπογράφεται ως ακολούθως:</p>${sigBlock(d.a_sign, "Ο Β’ Ανακριτικός Υπάλληλος", "Ο Ανακριτικός Υπάλληλος")}`;
...
function exportProkRights() {
    ...
    let body = `<p style="${pStyle}">Στην ${d.city} σήμερα την ${d.dateStr} και ώρα ${d.v("doc_start")}΄, ενώπιον εμού του ${d.anakr} υπηρετούντος στο ${d.deptFull}, παρισταμένου και του ${d.banakr} της ιδίας Υπηρεσίας προσληφθείς ως Β’ Ανακριτικού Υπαλλήλου, προσκλήθηκε ${d.a_o} κατωτέρω σημειούμεν${d.a_os.slice(1)} που ονομάζεται ${d.prof}, ${d.a_ton} οποί${d.a_o} ενημερώσαμε ότι κατηγορείται κατά παράβαση των διατάξεων: ${charge} και εξηγήσαμε σαφώς σ’ ${d.a_auton}, βάσει του άρθρου 95 του Κ.Π. Δικονομίας όλα τα εκ των άρθρων 244, 89, 90, 91, 92 παρ. 1, 95, 96, 99 παρ. 1 εδ. α, 2 και 4, 100, 101, 102, 103 και 104 του Κ.Π.Δ. δικαιώματά ${d.a_tou} και αναλυτικότερα:</p>
...
// 4. ΠΡΟΚΑΤΑΡΚΤΙΚΗ - ΕΝΙΑΙΑ (ΔΙΚΑΙΩΜΑΤΑ + ΑΝΩΜΟΤΙ) (ΕΠΙΣΗΜΟ ΠΡΟΤΥΠΟ)
function exportProkNoDeadline() {
    if (!validateRequiredFields(['surname', 'name', 'prok_abm', 'prok_charge', 'prok_plea'])) return;
    let d = getD(); let abm = d.v("prok_abm"); let charge = d.v("prok_charge"); let rightsAns = d.v("prok_rights_ans"); let pastAns = d.v("prok_past"); let plea = d.v("prok_plea");
    let midTime = d.v("doc_start"); let timeParts = midTime.split(":");
...
function exportProkDeadline() {
    ...
    <p style="${pStyle}">Ενταύθα γνωρίσαμε σ' ${d.a_auton} την πράξη που αφορά η εξέταση, ήτοι: Δυνάμει της ${abm}, ζητήθηκε η λήψη ανωμοτί (χωρίς όρκο) εξέτασης.</p>
