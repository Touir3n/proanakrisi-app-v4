...
// ==========================================
const pStyle = "text-align: justify; text-justify: inter-word; text-align-last: left; font-family: 'Times New Roman'; font-size: 12pt; line-height: 115%; margin: 0cm; padding: 0cm; background: white; color: black;";
...
function formatTextToParagraphs(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Κάθε γραμμή μπαίνει σε δικό της <p> για να λειτουργεί το text-align-last
...
function exportProkRights() {
    ...
    let d = getD(); let tm = getTimeRange("prok_rights_start", "prok_rights_end", "doc_start", "doc_end"); let abm = d.v("prok_abm"); let charge = d.v("prok_charge");
...
function exportProkNoDeadline() {
    ...
    <p style="${pStyle}">Για πιστοποίηση συντάχθηκε η παρούσα έκθεση, η οποία αφού αναγνώσθηκε and βεβαιώθηκε υπογράφεται ως ακολούθως:</p>
    ${sigBlock(d.a_sign, "Ο Β’ Ανακριτικός Υπάλληλος", "Ο Ανακριτικός Υπάλληλος")}`;
...
function exportProkAfterDeadline() {
    ...
    <p style="${pStyle}">Με μετά από τα παραπάνω προβήκαμε στην απαγγελία της αποδιδόμενης σ’ ${d.a_auton} αξιόποινης πράξης επί της οποίας καλείται να δώσει εξηγήσεις, ως ακολούθως:</p>
...
function exportRights() {
    ...
    let d = getD(); let rp = "text-align: justify; text-align-last: left; font-family: 'Times New Roman'; font-size: 11pt; line-height: 115%; margin: 0cm 0cm 4pt 0cm; padding: 0cm; background: white; color: black;";
    ...
    <b>2.</b> Την ίδια υποχρέωση έχει και ο ανακριτής, και τα ίδια δικαιώματα ο κατηγορούμενος, όταν κληθεί ξανά σε συμπληρωματική απολογία, πάντως μετά το τέλος της ανάκρισης & προτού διαβιβασθεί η δικογραφία στον Εισαγγελέα, καλείται πάντοτε ο κατηγορούμενος να μελετήσει όλη τη δικογραφία.</p>
...
function exportSeizure() {
    ...
    if (tType === "ΣΩΜΑΤΙΚΗΣ ΕΡΕΥΝΑΣ ΚΑΤΑΣΧΕΣΗΣ" || tType === "ΕΡΕΥΝΑΣ ΑΥΤΟΚΙΝΗΤΟΥ ΚΑΙ ΚΑΤΑΣΧΕΣΗΣ" || tType === "ΕΡΕΥΝΑΣ ΟΙΚΙΑΣ ΚΑΙ ΚΑΤΑΣΧΕΣΗΣ") {
        ...
        <p style="${pStyle}">- Προς πίστωση συντάχθηκε η παρούσα έκθεση, η οποία αναγνωσθείσα και βεβαιώθείσα υπογράφεται ως έπεται:</p>
