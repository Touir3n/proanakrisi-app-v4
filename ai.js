...
ΟΔΗΓΙΕΣ:
1. 1ο στοιχείο (Επώνυμο): Κράτα το ΚΕΦΑΛΑΙΑ, χωρίς τόνους.
2. 2ο στοιχείο (Όνομα): Κάντο Title Case (πρώτο γράμμα κεφαλαίο) και βάλε τόνο.
3. 3ο και 4ο (Πατρώνυμο/Μητρώνυμο): Title Case, βάλε τόνο, και ΜΕΤΑΤΡΕΨΕ ΤΑ ΣΕ ΓΕΝΙΚΗ ΠΤΩΣΗ (π.χ. ΔΗΜΗΤΡΙΟΣ -> Δημητρίου).
4. 5ο και 6ο (Τόπος Γέννησης & Περιοχή Κατοικίας): Κάντα Title Case και βάλε σωστούς τόνους. Διόρθωσε αν είναι όλα κεφαλαία (π.χ. ΑΘΗΝΑ -> Αθήνα, ΘΕΣΣΑΛΟΝΙΚΗ -> Θεσσαλονίκη).
5. 7ο, 8ο, 9ο (Δήμος, Οδός, Αρχή Έκδοσης): Title Case και σωστός τονισμός.
6. Δημιούργησε 4 ΝΕΑ στοιχεία στο τέλος:
   - 10ο: Το Επώνυμο σε ΓΕΝΙΚΗ ΠΤΩΣΗ (Κεφαλαία, χωρίς τόνους, π.χ. ΠΑΠΑΔΟΠΟΥΛΟΥ).
   - 11ο: Το Όνομα σε ΓΕΝΙΚΗ ΠΤΩΣΗ (Title Case, π.χ. Ιωάννη).
   - 12ο: Το Επώνυμο σε ΑΙΤΙΑΤΙΚΗ ΠΤΩΣΗ (Κεφαλαία, χωρίς τόνους, π.χ. ΠΑΠΑΔΟΠΟΥΛΟ).
   - 13ο: Το Όνομα σε ΑΙΤΙΑΤΙΚΗ ΠΤΩΣΗ (Title Case, π.χ. Ιωάννη).

Επίστρεψε ΜΟΝΟ τα 13 στοιχεία ενωμένα με το σύμβολο | , χωρίς καμία άλλη λέξη.`;
    ...
    if(result) {
        ...
        if(newVals.length >= 13) {
            fields.forEach((id, idx) => {
                let el = document.getElementById(id);
                if(el && newVals[idx]) { el.value = newVals[idx]; el.classList.remove('accent-warning'); saveMem(id); }
            });
            document.getElementById('surname_gen').value = newVals[9].toUpperCase(); saveMem('surname_gen');
            document.getElementById('name_gen').value = newVals[10]; saveMem('name_gen');
            document.getElementById('surname_acc').value = newVals[11].toUpperCase(); saveMem('surname_acc');
            document.getElementById('name_acc').value = newVals[12]; saveMem('name_acc');
        } else { 
