...
async function callGeminiAPI(prompt, buttonId, spinnerId) {
    ...
    let modelName = document.getElementById("gemini_model").value || localStorage.getItem("gemini_model") || "gemini-1.5-flash";
    ...
    try {
        ...
        if (!data.candidates || data.candidates.length === 0) { throw new Error("Το μοντέλο δεν επέστρεψε απάντηση (πιθανό μπλοκάρισμα)."); }
...
// ΕΔΩ ΕΓΙΝΕ Η ΒΑΣΙΚΗ ΑΛΛΑΓΗ ΣΤΟ PROMPT ΤΗΣ ΓΕΝΝΗΤΡΙΑΣ ΚΑΤΗΓΟΡΗΤΗΡΙΟΥ
async function generateChargeAI() {
    ...
    let prompt = `Ανάλαβε τον ρόλο ενός έμπειρου Αξιωματικού της Ελληνικής Αστυνομίας. 
Διάβασε το παρακάτω συμβάν: "${sanitizeForAI(text)}"

ΠΡΙΝ συντάξεις το κατηγορητήριο, ΠΡΕΠΕΙ ΝΑ ΑΞΙΟΛΟΓΗΣΕΙΣ ΥΠΟΧΡΕΩΤΙΚΑ:
1. Συρροή Αδικημάτων: Υπάρχουν πολλαπλά αδικήματα; (π.χ. Απειλή, Εξύβριση, Φθορά).
2. Ειδικοί Ποινικοί Νόμοι: Υπάρχει παράβαση του Νόμου Περί Όπλων (Ν. 2168/1993 - π.χ. αν αναφέρεται μαχαίρι ή άλλο όπλο), του Νόμου Περί Ναρκωτικών, Ενδοοικογενειακής Βίας κ.λπ.;
3. Δόλος/Απόπειρα: Αν υπήρξε επίθεση με όπλο, εξέτασε αν στοιχειοθετείται "Απόπειρα Επικίνδυνης Σωματικής Βλάβης" (ή άλλο σοβαρότερο αδίκημα) και όχι απλά "Απειλή".

Στη συνέχεια, ΣΥΝΤΑΞΕ το κατηγορητήριο ΑΥΣΤΗΡΑ με την εξής δομή:
[ΒΑΣΙΚΟ]
παράβαση των [Άρθρα Π.Κ. και Ειδικοί Νόμοι, π.χ. αρ. 333 ΠΚ, αρ. 1 & 14 Ν.2168/93], πράξη που έλαβε χώρα την [Ημερομηνία] και ώρα [Ώρα] στο/στην [Τόπος]
[ΠΕΡΙΣΤΑΤΙΚΑ]
Ειδικότερα, ανωτέρω τόπο και χρόνο, ο δράστης [σύντομη νομική διατύπωση των πραγματικών περιστατικών, περιγράφοντας τις πράξεις που στοιχειοθετούν τα παραπάνω αδικήματα]

Κανόνες:
- Μην προσθέσεις γεγονότα που δεν αναφέρονται στο κείμενο.
- Αν λείπει ημερομηνία, ώρα ή τόπος, άφησέ τα σε αγκύλες [].
- Επέστρεψε ΜΟΝΟ τις δύο ενότητες [ΒΑΣΙΚΟ] και [ΠΕΡΙΣΤΑΤΙΚΑ].`;

    let result = await callGeminiAPI(prompt, "btn_generate_charge", "spinner_charge");
    if (result) {
        try {
            let clean = result.replace(/```/g, "").trim();
            // Πιο "χαλαρά" regex για να πιάνουν σίγουρα τις ενότητες ακόμα και αν το Gemini βάλει bold
            let basicMatch = clean.match(/\[ΒΑΣΙΚΟ\]([\s\S]*?)\[ΠΕΡΙΣΤΑΤΙΚΑ\]/i);
            let detailsMatch = clean.match(/\[ΠΕΡΙΣΤΑΤΙΚΑ\]([\s\S]*)/i);
            
            let basic = basicMatch ? basicMatch[1].replace(/\*/g, "").trim() : "";
            let details = detailsMatch ? detailsMatch[1].replace(/\*/g, "").trim() : "";
        ...
        } catch (e) { alert("Σφάλμα ανάγνωσης από το AI."); }
    }
}
